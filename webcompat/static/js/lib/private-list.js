...
import issueTemplate from "templates/list-issue/issuelist-private.jst";
import { PaginationMixin } from "./mixins/private-pagination.js";
...
issueList.SortingView = Backbone.View.extend({
  ...
  render: function () {
    ...
    this.paginationDropdown
      .setElement(this.$el.find(".js-Dropdown-pagination"))
      .render();
...
issueList.IssueView = Backbone.View.extend(
  _.extend({}, issuesPagination, {
    ...
    updateIssues: function (category) {
      ...
      var issuesAPICategories = ["ml-autoclosed"];
      ...
      if (category && category.query) {
        ...
        if (!this._isLoggedIn) {
          ...
          this.issues.setURLState("/api/private/search", paramsCopy);
        }
      } else if (_.includes(issuesAPICategories, category)) {
        this.issues.setURLState(
          "/api/private/category/" + category,
          this.removeParamQuery(params)
        );
      } else {
        this.issues.setURLState("/api/private", this.removeParamQuery(params));
      }

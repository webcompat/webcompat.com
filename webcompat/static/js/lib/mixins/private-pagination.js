...
export function PaginationMixin() {
  ...
  this.requestNextPage = function (e) {
    ...
    if ((nextPage = this.model.getNextPage())) {
      ...
      const next = nextPage.replace("issues", "private");
      this.view.fetchAndRenderIssues({ url: next, instance: this });
    }
  };
  ...
  this.requestPreviousPage = function (e) {
    ...
    if ((prevPage = this.model.getPrevPage())) {
      ...
      this.view.updateModelParams(pageNum);
      // we pass along the entire URL from the Link header
      const prev = prevPage.replace("issues", "private");
      this.view.fetchAndRenderIssues({ url: prev, instance: this });
    }
  };
